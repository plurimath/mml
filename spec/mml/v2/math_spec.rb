...
RSpec.describe Mml::V2::Math do
  describe "round-trip" do
    ...
    it "parses math with nested mrow" do
      input = '<math xmlns="http://www.w3.org/1998/Math/MathML">' \
    ...
    it "preserves display attribute" do
      input = '<math xmlns="http://www.w3.org/1998/Math/MathML" display="block">' \
              "<mi>x</mi></math>"
      math = Mml::V2.parse(input)
      expect(math.display).to eq("block")
    end

    it "extracts typed child elements" do
      input = '<math xmlns="http://www.w3.org/1998/Math/MathML">' \
              "<mi>x</mi><mo>+</mo><mn>1</mn></math>"
      math = Mml::V2.parse(input)
