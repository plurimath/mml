...
|Deprecated (recognized but hidden) |`<mlabeledtr>`, `<none>` removed from `CommonElements` but classes still exist
|===
...
Mml::Configuration.register.register_model(MyCustomMi, id: :mi)
Mml::Math.new(...)

# After — explicit version
require "mml"
Mml::V3::Configuration.adapter = :nokogiri
Mml::V3::Configuration.register.register_model(MyCustomMi, id: :mi)
Mml::V3::Math.new(...)

# Or for MathML 4
Mml::V4::Configuration.adapter = :nokogiri
Mml::V4::Configuration.register.register_model(MyCustomMi, id: :mi)
Mml::V4::Math.new(...)
----
...
Each element self-registers in its version's `Configuration.register`.
...
`:mfrac`) resolved at parse time through `Lutaml::Model::Register`.

V4's `CommonElements` extends the base with the `<a>` hyperlink element.

=== Register and type resolution

When calling `from_xml` directly (outside of `Mml.parse` or `Mml::V3.parse`), pass
the version-specific register for correct type resolution:

[source,ruby]
----
Mml::V3::Math.from_xml(input, register: Mml::V3::Configuration.register)
Mml::V4::Math.from_xml(input, register: Mml::V4::Configuration.register)
----
...
# Access the version-specific register
Mml::V3::Configuration.register
Mml::V4::Configuration.register

# Assign a custom register
custom = Lutaml::Model::Register.new(:custom, fallback: [])
Mml::V3::Configuration.register = custom
----
